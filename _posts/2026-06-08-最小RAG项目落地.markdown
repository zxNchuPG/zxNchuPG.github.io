...
>
> [Microsoft Learn - Develop a RAG application with Spring AI and Azure OpenAI](https://learn.microsoft.com/en-us/training/modules/build-enterprise-ai-agents-with-java-spring/4-spring-ai-development/)
>
...
所以一个最小可行的 RAG 项目，我会建议至少具备下面 7 个能力：
...
如果你本身就是 Java 团队，我会更建议第一版从下面这条线起：
...
如果一开始就把这些都揉在一个 Service 里，后面你很难判断到底是哪一层出了问题。
...
如果你准备用 Spring AI 做第一版 RAG，我建议先把下面几个对象关系看明白。
...
如果你现在只做第一版，`QuestionAnswerAdvisor` 已经足够。
...
### 8.1 为什么第一版我更推荐 `pgvector`

因为它适合回答下面这个问题：

> 我已经是 Java + Spring Boot + PostgreSQL 团队了，能不能先不引入独立向量库？
...
如果你使用 Spring AI 的自动 schema 初始化，也可以让它帮你建，但要注意：

- 新版本需要显式开启 `initialize-schema`
- embedding 维度要和你的模型一致
...
如果你走的是 Azure OpenAI 或其他兼容实现，主要替换的是模型配置，不影响整体思路。

## 十、离线入库怎么做：最小实现先把文档读、切、存跑通

### 10.1 文档来源先收敛

第一版强烈建议你只支持 1 到 2 种输入格式，比如：
...
这里的重点不是 `vectorStore.add()` 这一行本身，而是你在入库前已经把：
...
这是 Spring AI 官方给的最直接 RAG 方式，也是我认为第一版最好用的方式。
...
这在多知识库、多租户场景里非常实用，因为你不用为每个知识库都 new 一套固定 client。

## 十二、如果你想更像“正经接口服务”，建议至少暴露这 3 类 API
...
如果没有这些信息，后面用户一说“答错了”，你基本只能靠猜。

所以我会建议第一版就加最基础的日志和调试开关。
...
## 十六、Java Spring Boot / Spring AI 的推荐落地方案，我会怎么选

如果你问我“现在真的要做一个 Java 版 RAG 服务，最推荐哪条路”，我会给出下面这个组合。
...
## 十九、如果我现在从 0 开一个 Java RAG 小项目，我会按这个顺序做
...
## 二十、最后收个口：最小项目真正要证明的，不是“我会调 AI API”

如果把这篇笔记压缩成一句话，我最想强调的是：

> **最小 RAG 项目要证明的，不是“模型能回答”，而是“你的文档能被稳定入库、稳定召回、稳定约束生成”。**

而在 Java Spring Boot 生态里，我认为最务实的起步方案就是：
