...
线程池中的核心线程数。当提交一个任务时，线程池创建一个新线程执行任务，直到当前线程数等于 `corePoolSize`；如果当前线程数已经达到 `corePoolSize`，继续提交的任务会被保存到阻塞队列中，等待被执行。
...
**为什么是“先放阻塞队列，队列满了再扩到 `maximumPoolSize`”，而不是“先一路创建线程，直到达到 `maximumPoolSize`，再把任务丢到阻塞队列”？**

先记结论：

**`ThreadPoolExecutor` 的默认设计，更偏向先控制线程数量、再利用队列做缓冲，只有队列也扛不住了，才把 `maximumPoolSize` 当作应急扩容上限。**

#### `corePoolSize / queue / maximumPoolSize` 的流程图
...
**核心线程先上 -> 队列先顶一下 -> 还顶不住再扩容 -> 再不行就拒绝。**

#### 为什么不是先扩线程，再用队列

##### 1. 线程是高成本资源，队列相对便宜

创建线程不仅仅是“new 一个对象”这么简单，还会带来：
...
所以线程池默认不希望任务一多就立刻疯狂扩线程，而是更倾向先用队列缓冲一下。

##### 2. `corePoolSize` 和 `maximumPoolSize` 本来就不是一个层次的容量

可以这样理解：

- `corePoolSize`：常备军，平时长期工作的线程数
- `workQueue`：缓冲区，用来削峰
- `maximumPoolSize`：预备队，只有缓冲区也扛不住时才启用

也就是说，`maximumPoolSize` 的设计初衷不是“线程池平时就应该跑到这个数量”，而是“出现持续压力时的扩容上限”。
...
- 线程数会迅速膨胀
- CPU 可能花更多时间在线程切换上
- 数据库、磁盘、网络这些后端资源压力也会被进一步放大

而先放队列，往往能把一小波突发流量平滑掉，不必为了一阵瞬时高峰创建大量线程。

##### 4. 先扩线程不一定更快，可能反而更慢

线程多不等于吞吐量一定高。

尤其在 CPU 密集场景下，如果线程数远大于 CPU 核数，结果常常是：

- 真正执行任务的效率没有明显提升
- 上下文切换明显变多
- 整体吞吐量反而下降

所以默认策略更强调“可控”和“稳定”，而不是“看见任务就先把线程开满”。
...
所以 `ThreadPoolExecutor` 选择的是：

- 优先把线程数量控制在 `corePoolSize`
- 用阻塞队列吸收突发请求
- 只有队列满了，才说明系统真的开始持续承压
- 这时再启用 `maximumPoolSize` 作为扩容手段

#### 那有没有更像“先扩线程”的线程池

有。

比如配合 `SynchronousQueue` 时，线程池几乎不缓存任务，任务来了要么直接交给线程，要么就创建新线程。

这时候线程池的行为就会更激进，看起来更接近：

**先尽量扩线程，再谈排队。**

这也是为什么 `newCachedThreadPool()` 往往显得更“猛”，但如果使用不当也更容易让线程数快速增长。

#### 面试里怎么回答

如果面试官问“为什么线程池是先队列满了再扩到 `maximumPoolSize`”，可以直接这样说：

**因为线程是高成本资源，线程池默认更重视资源可控和系统稳定性。`corePoolSize` 表示常态并发能力，阻塞队列用于缓冲突发任务，只有当队列也扛不住时，才继续扩到 `maximumPoolSize`。这样可以避免线程数过快膨胀，把 `maximumPoolSize` 留作系统承压时的应急扩容上限。**

### 3. `keepAliveTime`

线程空闲时的存活时间。默认情况下，只有当线程池中的线程数大于 `corePoolSize` 时，`keepAliveTime` 才会起作用；如果一个线程空闲的时间达到 `keepAliveTime`，则会终止，直到线程池中的线程数不超过 `corePoolSize`。但是如果调用了 `allowCoreThreadTimeOut(boolean)` 方法，`keepAliveTime` 参数也会起作用，直到线程池中的线程数为 `0`。

#### 线程什么时候会被销毁

这个问题也非常常见，可以先记结论：

**线程池里的线程不是任务一执行完就销毁，而是先回到线程池里继续等任务；只有在线程池认为“这条线程没必要继续留着了”时，才会回收它。**
...
当线程池里的线程数 **大于 `corePoolSize`** 时，多出来的那部分线程可以理解成“临时扩容线程”，它们不会永久保留。

如果这些线程在空闲状态下，超过了 `keepAliveTime` 还没有拿到新任务，就会被回收。

也就是说，默认情况下：

- 核心线程：默认常驻
- 非核心线程：空闲超过 `keepAliveTime` 后会销毁

##### 2. 开启 `allowCoreThreadTimeOut(true)` 后，核心线程也会被回收
...
那么核心线程也会像非核心线程一样，空闲超过 `keepAliveTime` 就被回收。
...
- `shutdown()`：不再接收新任务，但会继续处理队列中的任务，空闲线程会被中断唤醒并逐步退出
- `shutdownNow()`：尝试中断所有工作线程，并且不再处理队列里剩余任务

所以从结果上看，这两种方式最终都会让线程池中的线程结束，只是退出过程没有那么温和。

#### 为什么线程执行完任务不会立刻销毁

因为线程池的目标就是复用线程。

如果每执行一个任务就把线程销毁，下一个任务又重新创建线程，那就又退回成“来一个任务创建一个线程”的老路了，线程池就失去意义了。
...
2. 不立刻结束
3. 再去队列里尝试拿下一个任务
4. 只有长时间拿不到任务，或者线程池状态要求退出时，才结束

#### 用源码流程怎么理解

前面源码里的 `runWorker()` 和 `getTask()`，其实就是在做这件事：

- `runWorker()`：不断循环执行任务
- `getTask()`：继续从队列里取任务
- 如果 `getTask()` 最终返回 `null`，说明这个线程该结束了
- 之后进入 `processWorkerExit()` 做收尾清理

所以你可以把“线程销毁”理解成：

**不是任务执行完就销毁，而是 `getTask()` 判断这条线程已经没有继续存在的必要了。**
...
如果面试官问“线程池里的线程什么时候会被销毁”，可以这样答：

**线程池里的线程执行完任务后不会立刻销毁，而是会继续从工作队列获取新任务。默认情况下，超过 `corePoolSize` 的非核心线程在空闲时间超过 `keepAliveTime` 后会被回收；如果开启了 `allowCoreThreadTimeOut(true)`，核心线程也会因为空闲超时被回收。另外在线程池执行 `shutdown()` 或 `shutdownNow()` 时，工作线程也会逐步退出。**
...
任务缓存队列，用来存放等待执行的任务。如果当前线程数为 `corePoolSize`，继续提交的任务就会被保存到任务缓存队列中，等待被执行。
...
任务拒绝策略，当阻塞队列满了，且线程池中的线程数达到 `maximumPoolSize`，如果继续提交任务，就会采取任务拒绝策略处理该任务，线程池提供了 4 种任务拒绝策略（详细内容参考**任务拒绝（策略）**）：
