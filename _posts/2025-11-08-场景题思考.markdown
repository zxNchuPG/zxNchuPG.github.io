...
**延迟消息（主路径） + 定时任务扫描（兑底补偿）**

1. 主流程：下单时发送延迟消息，到期后触发关单
2. 兑底：定时任务每5分钟扫描一次，处理消息丢失/消费失败的漏网订单
...
自增ID直接编码，攻击者拿到一个短码就能推算相邻链接，有严重安全风险。解决思路：
...
```text
短码 = Base62(ID拼接2位随机数)
如：ID=12345, 随机数=78 → Base62(1234578) → "qR3x0a"
```
...
使用 Snowflake/UUID 等非连续ID，天然不可预测，但ID空间利用率低
...
> 建议设置过期时间（如90天），可减少存储压力，过期后短码可回收复用
