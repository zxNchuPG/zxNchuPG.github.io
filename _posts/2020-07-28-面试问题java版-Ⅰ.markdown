...
- **ArrayList：** 默认初始容量10，扩容为原来的1.5倍（`oldCapacity + (oldCapacity >> 1)`）
- **Vector：** 默认初始容量10，扩容为原来的2倍（也可通过构造函数指定增量`capacityIncrement`）
...
- **Vector：** 不推荐使用，需要线程安全可用 `Collections.synchronizedList()` 或 `CopyOnWriteArrayList`
