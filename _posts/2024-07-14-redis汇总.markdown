...
SET money 1000      // 修改了被监视的键[citation:1]
```
...
* ACID 特性：
  ...
  2. 隔离性：由单线程模型保证，事务执行过程中不会被其他操作打断。
...
原子性的保证：这是 Lua 脚本最核心的优势。当 Lua 脚本在 Redis 中运行时，在整个脚本执行期间，不会有其他任何命令被执行。这完美解决了事务无法依赖中间结果的问题
